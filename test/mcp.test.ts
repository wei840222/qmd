...
describe("MCP Server", () => {
  ...
  describe("qmd:// resource", () => {
    ...
    test("returns not found for missing document", () => {
      ...
      expect(doc == null).toBe(true); // SQLite bindings may return null or undefined
    });
...
describe.skipIf(!!process.env.CI)("MCP HTTP Transport", () => {
  ...
  async function mcpRequest(
    methodOrPayload: string | { jsonrpc?: string; id?: number; method?: string; params?: Record<string, unknown> },
    ...
    if (typeof methodOrPayload === "object" && methodOrPayload !== null) {
      ...
      finalParams = (methodOrPayload.params as Record<string, unknown>) ?? {};
      finalId = methodOrPayload.id ?? 1;
    } else {
      method = methodOrPayload;
      finalParams = params;
      finalId = id;
    }
    const toolName = typeof finalParams.name === "string" ? finalParams.name : undefined;
    ...
    const res = await fetch(`${baseUrl}/mcp`, {
      ...
      body: JSON.stringify({
        ...
        id: finalId,
        method,
        params: { ...finalParams, _meta: mcpMeta },
      }),
    });
