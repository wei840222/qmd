  ...
  describe("findDocuments (multi-get)", () => {
    ...
    test("findDocuments finds by docid", async () => {
      ...
      if (!("error" in doc)) {
        ...
        expect((docs[0]!.doc as any).docid).toBe((doc as any).docid);
      }
...
describe("cleanupOrphanedVectors atomicity", () => {
  ...
  async function seedOrphanFixture(store: Store): Promise<void> {
    ...
    const { identity, lease } = beginEmbeddingFixture(store, "test-model", 3);
    ...
    insertEmbedding(store.db, "keephash", 0, 0, new Float32Array([1, 2, 3]), "test-model", now, 1, identity.fingerprint, lease);
    insertEmbedding(store.db, "orphanhash", 0, 0, new Float32Array([4, 5, 6]), "test-model", now, 2, identity.fingerprint, lease);
    insertEmbedding(store.db, "orphanhash", 1, 10, new Float32Array([7, 8, 9]), "test-model", now, 2, identity.fingerprint, lease);
    completeEmbeddingBuild(store.db, lease);
  }
...
describe("Vector Search collection filter", () => {
  test("searchVec finds docs in a small collection crowded by a large one (#791, #803)", async () => {
    ...
    const dims = 8;
    const { identity, lease } = beginEmbeddingFixture(store, "test", dims);
    const now = new Date().toISOString();
    ...
    for (let i = 0; i < 250; i++) {
      ...
      embedding[0] = 1;
      insertEmbedding(store.db, hash, 0, 0, embedding, "test", now, 1, identity.fingerprint, lease);
    }
    ...
    insertEmbedding(store.db, targetHash, 0, 0, targetEmbedding, "test", now, 1, identity.fingerprint, lease);
    completeEmbeddingBuild(store.db, lease);
  ...
  test("searchVec multi-collection union is not starved by a third collection (#775)", async () => {
    ...
    kbEmbedding[0] = 0.55;
    kbEmbedding[1] = 0.84;
    insertEmbedding(store.db, kbHash, 0, 0, kbEmbedding, "test", now, 1, identity.fingerprint, lease);
    ...
    insertEmbedding(store.db, notesHash, 0, 0, notesEmbedding, "test", now, 1, identity.fingerprint, lease);
    completeEmbeddingBuild(store.db, lease);
